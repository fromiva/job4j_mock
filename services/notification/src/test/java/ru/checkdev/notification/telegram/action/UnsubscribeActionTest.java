...
class UnsubscribeActionTest {
    ...
    @BeforeEach
    void beforeEach() {
        unsubscribeAction = new UnsubscribeAction(tgDescCallWebClint, userTelegramService, subscribeTopicService);
    ...
    void handle() {
        ...
        when(subscribeTopicService.findTopicByUserId(userId)).thenReturn(List.of(topic.getId()));
        SendMessage actual = (SendMessage) unsubscribeAction.handle(message);
        assertThat(actual.getChatId()).isEqualTo(chatId.toString());
        assertThat(actual.getText()).startsWith("Выбирите номер темы для отдписки");
    }
    @Test
    void handleWhenNothingToDelete() {
        List<TopicDTO> topics = List.of(topic);
        when(userTelegramService.findByTelegramChatId(chatId)).thenReturn(Optional.of(userTelegram));
        when(tgDescCallWebClint.doGetList(any())).thenReturn(Mono.just(topics));
        when(subscribeTopicService.findTopicByUserId(userId)).thenReturn(List.of());
        SendMessage actual = (SendMessage) unsubscribeAction.handle(message);
        assertThat(actual.getChatId()).isEqualTo(chatId.toString());
        assertThat(actual.getText()).startsWith("У вас пока нет подписок");
    }
    ...
    void callbackWhenCorrectUserChatId() {
        ...
        when(userTelegramService.findByTelegramChatId(chatId)).thenReturn(Optional.of(userTelegram));
        when(subscribeTopicService.deleteAllByUserIdAndTopicId(userId, topic.getId())).thenReturn(1);
        SendMessage actual = (SendMessage) unsubscribeAction.callback(message);
        assertThat(actual.getText()).startsWith("Вы успешно отписаны от темы");
    }
    @Test
